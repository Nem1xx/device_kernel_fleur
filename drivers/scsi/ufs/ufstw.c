...
#define ATTR_IDN_NAME(idn)						\
	(idn == QUERY_ATTR_IDN_TW_FLUSH_STATUS ? "flush_status" :	\
	 idn == QUERY_ATTR_IDN_TW_BUF_SIZE ? "avail_buffer_size" :\
...
static void ufstw_alloc_shared_lu(struct ufsf_feature *ufsf, int lun, u8 *lu_buf)
{
	struct ufstw_lu *tw;
			   ...
			   int lun, u8 *lu_buf)
{
	if (ufsf->tw_dev_info.tw_buf_type == TW_BUF_TYPE_SHARED &&
	    !ufsf->tw_lup[0])
		ufstw_alloc_shared_lu(ufsf, lun, lu_buf);
...
static inline void ufstw_print_lu_flag_attr(struct ufstw_lu *tw)
{
	...
	INFO_MSG("tw_attr ufstw_lu[%s] IDN (0x%.2X) buffer_size (%u)",
		 lun_str, QUERY_ATTR_IDN_TW_BUF_SIZE,
